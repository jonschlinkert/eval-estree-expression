...
class ExpressionSync {
  ...
  constructor(tree, options = {}, { async = false, created = false } = {}) {
    ...
    if (options.allow_functions && !created) {
      const create = this.constructor.isAsync ? Functions : FunctionsSync;
      const Ctor = create(this.constructor);
      return new Ctor(tree, options, { async, created: true });
    }

    if (options.async && !async) {
      const Expression = require('./Expression');
      return new Expression(tree, options, { async: true, created });
    }
  }
  ...
  visit(node, context, parent) {
    ...
    if (typeof visitor !== 'function') {
      throw new TypeError(`visitor "${node.type}" is not implemented`);
    }
    ...
    const close = val => {
      if (val instanceof Promise) return val.then(v => close(v));
  ...
  isRegExpOperator(node, context)  {
    return node.operator === '=' && node.right?.operator === '~' && this.options.regex_operator !== false;
  }
  ...
  comparison(left, operator, right, context, node) {
    const bool = this.options.boolean_logical_operators === true;
  ...
  BinaryExpression(node, context) {
    ...
    if (left instanceof Promise) {
      return left.then(v => this.comparison(v, operator, right, context, node));
    }

    return this.comparison(left, operator, right, context, node);
  }
  ...
  Identifier(node, context, parent) {
    ...
    if (context != null) {
      ...
      if (hasOwnProperty.call(context, node.name)) return;
    }
    ...
    if (this.options.strict === true && this.options.allow_functions !== true) {
      throw new TypeError(`Cannot read property '${node.name}' of undefined`);
    }
  }
  ...
  ObjectExpression(node, context) {
    ...
    for (const property of node.properties) {
      ...
      if (type === 'SpreadElement') {
        ...
        object[key.value || key.name] = this.visit(value, context, property);
      }
    }
