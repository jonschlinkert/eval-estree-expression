...
class ExpressionSync {
  ...
  unset(node, context) {
    if (node.type === 'Identifier') {
      ...
      if (this.options.prefix && !name.startsWith(this.options.prefix)) {
        name = `${this.options.prefix}.${name}`;
      }

      if (context != null) {
        deletePath(context, name);
        delete context[name];
      }
  ...
  Identifier(node, context, parent) {
    ...
    if (context != null) {
      if (this.options.prefix && utils.isPlainObject(context) && !context[this.options.prefix]) {
        context = { ...context, [this.options.prefix]: context };
      }
